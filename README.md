...
**해결책:** 
- 릴레이 조깅 - 이전 주자 도착해야 다음 주자 출발 (강제성 + 탈락 방지)
- 역할 전환- 한 사람이 계속 주자의 역할을 하지 않고 다른 사람의 동반자로 전환
...
#### 릴레이 방식
- **이전 주자**가 중간 지점 도착 → **다음 주자**와 합류 → 이전 주자가 릴레이 버튼
- 버튼 누르는 순간: 다음 사람 타이머 시작, 두 사람이 함께 달림 (전의 주자가 동반자로 역할 바꿈)
...
- 아이디/비밀번호 입력
- JWT 토큰 기반 인증 시스템
- 로그인 성공 시 토큰 저장 및 자동 로그인 유지
- 로그인 실패 시 명확한 오류 메시지 표시
- "회원가입" 링크로 신규 사용자 가입 유도

**보안:**
- 비밀번호는 bcrypt로 해싱되어 저장 (saltRounds: 10)
- JWT 토큰으로 세션 관리
...
**기본 정보:**
- **아이디**: 중복 확인 기능 포함
- **비밀번호**: bcrypt 해싱 처리 (saltRounds: 10)
- **닉네임**: 서비스 내 표시 이름
- **이메일**: 알림 및 계정 복구용
- **전화번호**: 긴급 연락용
- **주소**: Daum 우편번호 API 연동으로 정확한 주소 검색

**건강 정보 (중요):**
- **공병/기저질환**: 주요 목적은 달리기에 적합하지 않은 사람과 일정 계획에서 문제가 있을 수 있는 사람을 팀장이 참고할 수 있도록 선별하는 것
...
팀장 신청은 코스 90% 이상 무결석(신용도 양호) 조건을 충족해야 하며, 경로, 강도, 시간을 직접 설계.
...
1. **서비스 로직상 모바일 앱이어야 함** - 웹 기반의 한계 (거의 불가능)
2. **지도 API 미구현** 
3. **완료/미완료 구분 오류** - 기록에서 모두 '완료'로 표시됨
4. **리더의 완성율이 90%이하로 떨어질 때**-punishment조치
5. **익명 피드 응급 조치 기능 없음** - 위기 상황 대응 불가
6. **댓글 태이블에 replyToCommentId 추가**-notification의 목적은 알림이기 보다 쉽게 일정처리->없으면 해당 댓글에 갈 수 없음.

### 🟡 Important

7. **팀장 권한 부족** - 경로/시간/멤버 수정 불가
8. **팀원 탈퇴 기능 없음**
9. **지역 필터링 하드코딩** 
10. **릴레이 중 알림 미구현**
...
11. **비밀번호 찾기 미구현**
12. **개인 인증 없음** - 실명 인증 시스템 부재
13. 팀 가입 신청 시 건강 상태란 중복 O
