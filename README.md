...
**기본 정보:**
- **아이디 (userId)**: 중복 확인 기능 포함
- **비밀번호 (password)**: bcrypt 해싱 처리 (saltRounds: 10)
- **닉네임 (nickname)**: 서비스 내 표시 이름
- **이메일 (email)**: 알림 및 계정 복구용
- **전화번호 (phone)**: 긴급 연락용
- **주소 (addr)**: Daum 우편번호 API 연동으로 정확한 주소 검색
  - 자동으로 구 단위까지 파싱되어 팀 추천 시 활용
- **생년월일 (birth)**: 연령대별 맞춤 서비스 제공
- **성별 (gender)**: 통계 및 서비스 개선용

**건강 정보 (중요):**
- **공병 (comorbidity)**: 당뇨, 심장병 등 동반 질환
- **기저질환 (underlying_condition)**: 천식, 고혈압 등 기존 질환

> **💡 왜 건강 정보가 필요한가요?**
> 
> 팀장이 팀원 구성 시 참고하여 안전한 릴레이 환경을 만들기 위함입니다:
> - 심한 천식 → 고강도 구간 배정 피함
> - 당뇨 환자 → 저혈당 위험 고려, 이른 아침/늦은 시간 배정 주의
> - 심장 질환 → 급경사 구간 제외
> - 무릎/관절 문제 → 거리 조절 필요
>
> 이를 통해 **모두가 안전하게** 참여할 수 있는 환경을 조성합니다.
